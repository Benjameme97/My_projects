---
title: "Statistical simulation and power analysis"
...
I have decided to investigate the data from this study. The sample size is suspiciously small, while the  statistical results are reported to be significant. I want to check the power of their analysis and see what is the probability of arriving to significant results with simulated data of the same parameters (N, M, SD).


Data for siblings' rivalry 
...
Data for siblings' conscientiousness 
...
```
Testing simulated data with equal parameters to check if the simluation runs properly
...
The proportion of significant results from the original data tends to converge to less than 5 %. This is probably caused by using ANOVA where one of the two compared groups has huge variance. In combination with a rather small number of DoF (computed from the sample size), the ANOVA rarely arrives to significant results.
